...
interface EditDramaFormProps {
  drama: Drama
  allPeople: Person[]
}

export default function EditDramaForm({ drama, allPeople }: EditDramaFormProps) {
  const router = useRouter()
  const [title, setTitle] = useState(drama.title)
  const [details, setDetails] = useState(drama.details)
  const [severity, setSeverity] = useState(drama.severity)
  const [selectedParticipants, setSelectedParticipants] = useState<Set<string>>(
    new Set(drama.participants.map((p) => p.id))
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'Failed to update drama')
        return
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex gap-3">
        <button
          ...
          onClick={() => router.push('/')}
          disabled={isSubmitting}
          className="flex-1 bg-gray-200 text-gray-800 py-2 px-4 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 disabled:bg-gray-100 disabled:cursor-not-allowed transition-colors"
        >
          Cancel
        </button>
        <button
        ...
        >
          {isSubmitting ? 'Updating...' : 'Update Drama'}
        </button>
      </div>
    </form>
  )
